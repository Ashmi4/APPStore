...
$a_name= mb_convert_encoding($appname, "UTF-8", "HTML-ENTITIES");
		$result45[] = array("IS Fav" => $a,"id" => $ref,"cat_name" => $name,"appname" => $a_name,"image" => $app_image,"version" => $version,"rank" => $rank,"download" => $download, "created_app" => $created_app);  
	
	$c3 =count($result45);
				
		}
			$qur = mysql_query("SELECT ref,appid,appname,app_image,app_video,screenshot,app_description,version,price,rank,count,download,created_app,id,name,description FROM app_master JOIN category_master ON category_master.id=app_master.appid WHERE category_master.id='$id' order by rank desc LIMIT $limit,$start");
...
$a_name= mb_convert_encoding($appname, "UTF-8", "HTML-ENTITIES");
	...
	$c1 =count($result44);
			...
			$qur = mysql_query("SELECT ref,appid,appname,app_image,app_video,screenshot,app_description,version,price,rank,count,download,created_app,id,name,description FROM app_master JOIN category_master ON category_master.id=app_master.appid order by rank desc LIMIT $limit,$start");
