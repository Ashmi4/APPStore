...
<script type="text/javascript" src="<?php echo base_url(); ?>assets/js/jquery-1.9.0.min.js"></script>
 ...
 <script type="text/javascript" src="<?php echo base_url(); ?>assets/js/paging.js"></script><style type="text/css">  
 .pg-normal {                color: black;                font-weight: normal;                text-decoration: none;  
 cursor: pointer;                }            .pg-selected {                color: black;                font-weight: bold;
...
<?php } ?>
					</div>							  						 <div class="clear"></div>					</div>						  <script type="text/javascript">								function DropDown(el) {									this.dd = el;									this.initEvents();								}								DropDown.prototype = {									initEvents : function() {										var obj = this;															obj.dd.on('click', function(event){											$(this).toggleClass('active');											event.stopPropagation();										});										}								}													$(function() {														var dd = new DropDown( $('#dd') );														$(document).click(function() {										// all dropdowns										$('.wrapper-dropdown-2').removeClass('active');									});													});		
       ...
       <li>
		...
		<li class="no-sub">
            <a class="top-heading " href="<?php echo base_url();?>Apps/top">Top Rated Apps</a>
						...
						<div class="heading">		                                 
						<?php			   $i=1; 		   foreach($cat_app->Result() as $row) { ?>  
...
<?php 
   ...
   ?>		
	...
	<a href="<?php echo base_url(); ?>Apps/details/<?php echo $row->ref; ?>">
<img src="<?php echo $row->app_image; ?>" alt="" /></a>				
	<h5 class="mess"><?php echo $row->appname; ?></h5>	
			...
			if (!empty($_SESSION["uName"])){ ?>
			 ...
			 <button onClick="document.location.reload(true)"><img src="<?php echo base_url(); ?>assets/images/black_wish.png"/>
									Add To Wishlist 
									</button>
			 <?php } ?>
		 <?php } else{ ?>
		  ...
		  <?php } ?>
							  ...
							  <script>function myFunction() {    document.getElementById("demo").innerHTML = "Browse Wishlist";}</script>	
							  <div id="pageNavPosition"></div>	
							  <script type="text/javascript"><!--        var pager = new Pager('results', 3);         pager.init();         pager.showPageNav('pager', 'pageNavPosition');         pager.showPage(1);    //--></script>				
							  </div>	
							  </div>	
							  </div>
